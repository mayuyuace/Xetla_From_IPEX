...
static void mm_mxfp4_out_marlin(
    torch::Tensor& out,
    const torch::Tensor& input,
    const torch::Tensor& weight,
    const torch::Tensor& weight_scl,
    const int64_t group_size) {
  using dtype_a = sycl::ext::oneapi::bfloat16;
  using dtype_b = uint32_t;
  using dtype_c = sycl::ext::oneapi::bfloat16;
  using dtype_scale = uint8_t;
  ...
  launch_hgemm_mxfp4_marlin<dtype_a, dtype_b, dtype_c, dtype_scale>(
...
static void group_mm_mxfp4_out_marlin(
    torch::Tensor& out,
    const torch::Tensor& input,
    const torch::Tensor& weight,
    const torch::Tensor& weight_scl,
  ...
  launch_group_hgemm_mxfp4_marlin<dtype_a, dtype_b, dtype_c, dtype_scale>(
      static_cast<dtype_c*>(out.data_ptr()),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("group_mm", &group_mm_mxfp4_out_marlin, "Marlin Mxfp4 GEMM");
  m.def("mm", &mm_mxfp4_out_marlin, "Marlin Mxfp4 GEMM");
}
